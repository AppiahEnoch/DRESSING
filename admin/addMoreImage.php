...
try {
    ...
    if ($newFilePath) {
        $sql = "INSERT INTO file (productid, filepath) VALUES (?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("is", $productId, $newFilePath);
        $stmt->execute();

        echo json_encode(['status' => 'success', 'message' => 'Image added successfully', 'filepath' => $newFilePath]); 
    } else {
        echo json_encode(['status' => 'error', 'message' => 'Image addition failed']);
    }
} catch (Exception $e) {
